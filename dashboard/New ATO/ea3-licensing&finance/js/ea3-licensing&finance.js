...
btn.onclick = function() {
 ...
 setTimeout(() => {
 modal.querySelector('.ea3-modal-content').style.transform = 'translate(-50%, -50%) translateY(0)'; 
 }, 10); // Timeout to allow CSS to catch up
}

// When the user clicks on <span> (x), slide the modal back down
span.onclick = function() {
 modal.querySelector('.ea3-modal-content').style.transform = 'translate(-50%, -50%) translateY(100vh)'; // Move down
 setTimeout(() => {
...
function updateButtonClass() {
 const inputElement = document.querySelector('.accordion-button input[type="checkbox"]');
...
document.querySelector('.accordion-button input[type="checkbox"]').addEventListener('change', updateButtonClass);
